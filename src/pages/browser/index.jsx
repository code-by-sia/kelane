            ...
            <form onSubmit={handleSubmit} className="flex-1 flex items-center gap-1 sm:gap-2 min-w-0">
              <div className="flex-1 relative min-w-0">
                {usingProxy && targetUrl ? (
                  <ShieldOffIcon size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-amber-500" />
                ) : (
          ...
          <div className="flex-1 relative overflow-hidden">
            {!targetUrl && (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-muted-foreground">
                ...
                <p className="text-sm">
                  Navigate to a recipe page, then click <strong>Extract Recipe</strong>.
                </p>
                <div className="flex flex-wrap justify-center gap-2 max-w-md">
                  {QUICK_SITES.map((site) => (
                    <Button key={site} variant="outline" size="sm" onClick={() => navigate(`https://${site}`, usingProxy)}>
                      {site}
                    </Button>
                  ))}
                </div>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center px-6">
                <div className="w-full max-w-sm flex flex-col gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="outline" size="sm" onClick={() => window.open(targetUrl, "_blank")}>
                      Open in new tab
                    </Button>
                  </div>
                </div>
